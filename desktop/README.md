...
- Linux `.deb` 会出现在应用菜单；`.tar.gz` / AppImage 第一次启动会自动创建桌面和应用菜单快捷方式
- macOS 请把 App 拖进「应用程序」
...
# Debian/Ubuntu
sudo apt install ./DeepSeek-0.1.7-linux-amd64.deb
```

Windows：下载 `DeepSeek-0.1.7-win.exe`。若 SmartScreen 提示未签名，选「更多信息 → 仍要运行」。
...
Download installers from the public [GitHub Release](https://github.com/zhuquan7237/zhuquan7237.github.io/releases/tag/desktop-v0.1.7). Anyone can download those files without signing in. GitHub Actions artifacts are not a public store.
