...
export function desktopFacts(env = process.env) {
    return {
...
export function parseDisabled(patch) {
...
export function renderBlock(disabled) {
...
export function mergeBlock(existing, disabled) {
 ...
 */
export function shellPluginsOf(patch) {
    const found = [];
    let rowId = "";
...
export function inventoryOf(manifest, patch) {
 ...
 */
export function toggleRowId(inv, key) {
    if (inv.plugins.some((plugin) => plugin.packageName === key))
        return key;
 ...
 */
export function apply(ctx, config = {}, hooks = {}) {
    ...
    const handlers = {
        ...
        [`${ROUTE_PREFIX}/toggle`]: async (req, res) => {
            ...
            if (!key)
                return sendJson(res, 400, { ok: false, message: "缺少插件标识" });
            const inv = await inventory();
            const packageName = toggleRowId(inv, key);
            if (!packageName)
                return sendJson(res, 404, { ok: false, message: "这个插件不在当前 profile 里" });
