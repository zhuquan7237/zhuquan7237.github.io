...
/** What a `hello` should replay from, and whether the caller is behind. */
export interface HelloResume {
  /** Where to resume: clamped into `[0, seq]`. */
  since: number
  /** The watermark was above the counter — it cannot be trusted at all. */
  stale: boolean
  /** Frames were missed (stale watermark, or the buffer no longer holds them). */
  gap: boolean
}
 ...
 */
export function normalizeSince(rawSince: number, seq: number, oldestBuffered?: number): HelloResume {
