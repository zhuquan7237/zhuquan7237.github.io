...
/** One connected socket plus its frame-parsing leftovers. */
export interface WsConnection {
  /** Send one text frame. Returns false when the socket is gone. */
  send(text: string): boolean
  /** Close with an optional code/reason. */
  close(code?: number, reason?: string): void
  /** True while the socket is usable. */
  readonly open: boolean
  /** Bytes seen from this client, for coarse abuse limits. */
  readonly received: number
}

/** What the bridge does with one connection's messages. */
export interface WsHandlers {
  /** A text frame from the client (JSON, by contract). */
...
export function encodePongFrame(payload: Buffer): Buffer {
  if (payload.length > 125) return Buffer.from([0x8a, 0])
...
): { frames: { opcode: number; payload: Buffer }[]; rest: Buffer<ArrayBufferLike> } {
...
export function acceptUpgrade(req: IncomingMessage, socket: Duplex, head: Buffer, handlers: WsHandlers): WsConnection | undefined {
  ...
  socket.on('data', (chunk: Buffer<ArrayBufferLike>) => {
    ...
    for (const frame of decoded.frames) {
      ...
      if (frame.opcode === 0x1) handlers.onMessage(connection, frame.payload.toString('utf8'))
