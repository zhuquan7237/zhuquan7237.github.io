...
export const SCOPES = ['read', 'prompt', 'config', 'admin'];
...
export function readStore(env = process.env) {
    try {
        const raw = JSON.parse(readFileSync(storePath(env), 'utf8'));
        return {
            version: 1,
...
export function writeStore(store, env = process.env) {
...
export function storeExists(env = process.env) {
...
export function issuePairing(store, now = Date.now()) {
    const pairing = { code: newPairCode(), expiresAt: now + PAIR_CODE_TTL_MS, attempts: 0 };
    store.pairing = pairing;
    return pairing;
}
/** Consume a pairing attempt: expired, exhausted and wrong all refuse. */
export function consumePairing(store, typed, now = Date.now()) {
    const pairing = store.pairing;
    if (pairing === undefined)
        return { ok: false, reason: '电脑端还没有生成配对码' };
    if (now > pairing.expiresAt)
        return { ok: false, reason: '配对码已过期，请在电脑端重新生成' };
    if (pairing.attempts >= PAIR_ATTEMPT_LIMIT)
        return { ok: false, reason: '尝试次数过多，请在电脑端重新生成配对码' };
...
export function deviceForToken(store, token) {
...
export function scopeAllows(scopes, method) {
    ...
    if (needed === 'read')
        return scopes.some((scope) => scope === 'prompt' || scope === 'config' || scope === 'admin');
    if (needed === 'prompt')
        return scopes.some((scope) => scope === 'config' || scope === 'admin');
