...
async function readRawBody(req, limit) {
    ...
    const iterator = req[Symbol.asyncIterator];
    if (iterator === undefined)
        return { ok: false, code: 'E_EMPTY', message: '文件内容为空' };
    for await (const chunk of req) {
        const buffer = typeof chunk === 'string' ? Buffer.from(chunk, 'utf8') : chunk;
        size += buffer.length;
        if (size > limit) {
...
export function apply(ctx, config = {}) {
     ...
     */
    const rememberGenerated = (sessionId, cwd, names) => {
        let added = 0;
        const rec = sessionRec(sessionId, cwd);
        const gen = (rec.generated ??= {});
        for (const name of names) {
            if (!GENERATED_FILE_RE.test(name) || gen[name] !== undefined)
                continue;
            try {
                    ...
                    if (hits !== null && hits.length > 0) {
                        void (async () => {
                            try {
                                ...
                                if (cwd !== '')
                                    rememberGenerated(sessionId, cwd, hits);
                            }
                            catch {
                                // 同上：旁路记账
                            }
    ...
    try {
        ctx.on('agent/assistant-stream', ((...args) => {
                ...
                if (attemptId !== '') {
                    if (frame.type === 'start') {
                        ...
                        if (assistantAttempts.size > 256) {
                            ...
                            if (oldest !== undefined)
                                assistantAttempts.delete(oldest);
                        }
                    }
                    else {
            ...
            if (sessionId === '' && attemptId !== '') {
                ...
                if (cut > 0)
                    sessionId = attemptId.slice(0, cut);
            }
            if (sessionId === '')
                return;
...
(async () => {
  try { net = (await (await fetch('${LOCAL_PREFIX}/network')).json()).network; netRender(); }
                ...
                void (async () => {
                    try {
                        ...
                        if (sub === 'add') {
                            ...
                            saveTransfer(store);
                            if (added.length > 0)
                                log(`隔空传输：已放入 ${added.length} 个文件，等待手机收取`);
    ...
    const sanitizeTransferName = (raw) => {
    ...
    const mimeForTransfer = (name) => TRANSFER_MIME[extname(name).replace('.', '').toLowerCase()] ?? 'application/octet-stream';
    const loadTransfer = () => {
        try {
            const raw = JSON.parse(readFileSync(transferIndexPath(), 'utf8'));
            const items = Array.isArray(raw?.items) ? raw.items : [];
            return { items: items.filter((x) => isRecord(x) && typeof x.id === 'string') };
        }
        catch {
            return { items: [] };
        }
    ...
    const slimSession = (item) => {
        ...
        if (values.title !== undefined)
            slimValues.title = values.title;
        if (isRecord(values.modelSelection))
            slimValues.modelSelection = values.modelSelection;
        const out = {
    ...
    const loadSessionsBundle = async () => {
        const now = Date.now();
        if (sessionsListCache !== null && now - sessionsListCache.at < SESSIONS_CACHE_MS) {
            return sessionsListCache;
        }
        const listed = (await callEngine(ctx, 'session.list', {}));
        const full = { ok: true, ...withFileCounts(listed) };
        const rawItems = Array.isArray(full.items) ? full.items : [];
        const lite = { ok: true, items: rawItems.map(slimSession) };
        const entry = { at: now, full, lite };
        sessionsListCache = entry;
        return entry;
    };
    /** session.search 的命中只带 sessionId/snippet：用列表行补全标题/时间/模型，保持相关性顺序。 */
    const enrichSearchHits = async (found) => {
        const out = (isRecord(found) ? found : { items: [] });
        const items = out.items;
        if (!Array.isArray(items) || items.length === 0)
            return out;
        const bundle = await loadSessionsBundle();
        const rows = Array.isArray(bundle.full.items) ? bundle.full.items : [];
        const byId = new Map();
            ...
            if (action === 'files') {
                ...
                if (scanned === undefined || scanned.genScanned !== true) {
                    try {
                        ...
                        if (found.size > 0)
                            rememberGenerated(sessionId, cwd, found);
        ...
        handler: (req, res) => {
            void (async () => {
                ...
                if (rel.startsWith('@generated/')) {
                    ...
                    if (GENERATED_FILE_RE.test(name))
                        file = join(GENERATED_DIR, name);
                }
                else {
                    const cwd = await sessionCwdOf(sessionId);
                    if (cwd === '') {
