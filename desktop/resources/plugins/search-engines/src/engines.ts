...
/** Engines a user can enable. Names are stable — they are written into settings. */
export type EngineKind = 'pool' | 'searxng' | 'tavily' | 'serper' | 'deepseek' | 'custom'

/** One engine's saved configuration. Every field is optional: presets fill the rest. */
export interface EngineConfig {
  enabled?: boolean
  /** Endpoint base. Meaning differs per engine and the preset says which. */
  baseURL?: string
  /** Credential reference resolved through the credentials service. */
  apiKeyEnv?: string
  /** Literal key, used when the credentials service holds none. */
  apiKey?: string
  /** Result count this engine should ask for. */
  maxResults?: number
  /** Tavily/Serper-style retrieval depth. */
  searchDepth?: string
  /** SearXNG language filter, e.g. `zh-CN`. */
  language?: string
  /** SearXNG safesearch level. */
  safesearch?: number
  /** Serper region/language hints. */
  gl?: string
  hl?: string
  /** Official DeepSeek: searches served inside one request. */
  maxUses?: number
  /** Custom engine: HTTP method. */
  method?: 'GET' | 'POST'
  /** Custom engine: URL template with `{query}` and `{maxResults}`. */
  urlTemplate?: string
  /** Custom engine: extra headers, `Name: value` per line. */
  headerLines?: string
  /** Custom engine: dot path to the results array, e.g. `data.items`. */
  resultsPath?: string
  /** Custom engine: dot paths inside one result. */
  urlPath?: string
  titlePath?: string
  snippetPath?: string
}

/** What a user sees before changing anything. */
export interface EnginePreset {
  kind: EngineKind
  label: string
  /** One plain sentence: what this engine is and when to pick it. */
  hint: string
  /** Defaults written the first time the page loads. */
  defaults: EngineConfig
  /** True when the engine cannot work without a credential. */
  needsKey: boolean
  /** Plain-language description of what `baseURL` must be for this engine. */
  urlHint: string
  /** True for engines that must receive an Anthropic-compatible Messages base. */
  anthropicBase?: boolean
  /** Credential name suggested when the user configures a key. */
  keyRef?: string
}
...
export function presetFor(kind: string): EnginePreset | undefined {
  return ENGINE_PRESETS.find((preset) => preset.kind === kind)
}

/** Result shape handed back through the seam; `snippet` is optional by contract. */
...
/** A search attempt's outcome, engine-agnostic. */
export interface EngineOutcome {
  sources: SearchSource[]
  content?: string
  /** Endpoint actually requested — shown when a test fails, so a wrong base is visible. */
  endpoint: string
}
...
export function clampResults(value: unknown, fallback = 8): number {
 ...
 */
export function proxyFor(url: string, proxy: string | undefined): string | undefined {
...
export function baseOf(config: EngineConfig | undefined, fallback: string): string {
  const raw = (config?.baseURL ?? '').trim() || fallback
  return raw.replace(/\/+$/, '')
}

/** Parse `Name: value` header lines into a record, ignoring blanks and comments. */
export function parseHeaders(lines: unknown): Record<string, string> {
...
/** One request an engine will issue, before credentials are attached. */
export interface EngineRequest {
  url: string
  init: { method: string; headers: Record<string, string>; body?: string }
  /** True when the engine cannot proceed without a key. */
  needsKey: boolean
  /** Credential reference this engine resolves, when it needs one. */
  keyRef?: string
}
 ...
 */
export function buildRequest(kind: EngineKind, config: EngineConfig | undefined, query: string, maxResults: number): EngineRequest {
  ...
  switch (kind) {
    ...
    case 'searxng': {
      ...
      if (config?.safesearch !== undefined) params.set('safesearch', String(config.safesearch))
      return {
    ...
    default: {
      ...
      const headers: Record<string, string> = { accept: 'application/json', ...parseHeaders(config?.headerLines) }
...
export function mapResponse(kind: EngineKind, config: EngineConfig | undefined, raw: unknown): EngineOutcome {
  ...
  if (kind === 'pool' || kind === 'tavily') {
    ...
    if (Array.isArray(answers) && answers.length > 0) content = text(answers[0])
  } else if (kind === 'serper') {
    ...
    if (!Array.isArray(blocks)) throw new Error('返回里没有 content 数组')
    ...
    for (const block of blocks as Record<string, unknown>[]) {
      ...
      for (const item of items as Record<string, unknown>[]) {
        ...
        sources.push({
          ...
          ...(text(item.page_age) ? { publishedAt: text(item.page_age) as string } : {}),
        })
      }
    }
    if (sources.length === 0) throw new Error('返回里没有 web_search_tool_result（这次请求没有触发服务端搜索）')
  } else {
    const rows = readPath(raw, config?.resultsPath ?? 'results')
    if (!Array.isArray(rows)) throw new Error(`在 ${config?.resultsPath ?? 'results'} 处没找到数组`)
    for (const row of rows) {
...
export function looksLikeWrongProtocol(kind: EngineKind, config: EngineConfig | undefined): string | undefined {
  ...
  if (kind === 'deepseek') {
    ...
    if (/\/messages\/?$/.test(base)) return '地址不应该包含 /messages，只填到 /v1 这一层就行。'
    return undefined
  }
 ...
 */
export function explainFailure(kind: EngineKind, status: number | undefined, detail: string | undefined): string {
  const preset = presetFor(kind)
  const name = preset?.label ?? kind
  const tail = detail && detail.trim() !== '' ? `（服务端说明：${detail.trim().slice(0, 160)}）` : ''
  if (status === undefined) {
    const lowered = String(detail ?? '').toLowerCase()
    if (/abort|timeout|timed out/.test(lowered)) return `${name} 请求超时 —— 检查网络/代理，以及该实例是否在运行。`
    if (/econnrefused|fetch failed|connect/.test(lowered)) return `${name} 连不上 —— 实例没启动、端口不对，或需要代理。`
    return `${name} 请求失败：${detail ?? '未知错误'}`
  }
  if (status === 401 || status === 403) return `${name} 拒绝了这个 Key（HTTP ${status}）—— 去拿一个有效 Key 填进这张卡，或改用不需要 Key 的引擎。${tail}`
  if (status === 402) return `${name} 提示余额/配额不足（HTTP 402）—— 充值、换 Key，或换引擎。${tail}`
  if (status === 429) return `${name} 触发限流（HTTP 429）—— 等一会儿、降低频率，或在优先级里把它排到后面。${tail}`
  ...
  if (status >= 500) return `${name} 服务端出错（HTTP ${status}）—— 稍后重试或换引擎。${tail}`
 ...
 */
export function attemptOrder(order: readonly string[] | undefined, engines: Record<string, EngineConfig | undefined>): EngineKind[] {
  const enabled = (Object.keys(engines) as EngineKind[]).filter((kind) => engines[kind]?.enabled === true)
  const preferred = (order ?? []).filter((kind): kind is EngineKind => enabled.includes(kind as EngineKind))
  const rest = enabled.filter((kind) => !preferred.includes(kind))
  return [...preferred, ...rest]
}

/** One engine's failure inside a multi-engine attempt. */
...
export function summarizeFailures(failures: readonly AttemptFailure[]): string {
