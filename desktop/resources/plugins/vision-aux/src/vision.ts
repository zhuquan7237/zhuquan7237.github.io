...
export const DEFAULT_VISION_PROMPT =
  '你是编程助手的视觉辅助模型。请详细描述这张图片，供另一个无法查看图片的模型使用：' +
...
/** Resolved options after the plugin's `apply` fills defaults. */
export interface VisionAuxOptions {
  /** OpenAI-compatible base URL; `/chat/completions` is appended. */
  baseURL: string
  /** API key sent as `Authorization: Bearer`. */
  apiKey: string
  /** Vision model id, e.g. `qwen-vl-max` or `gpt-4o-mini`. */
  model: string
...
/** One image as returned by `ctx.attachments.readImage`. */
...
export async function describeImage(
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (body.trim().length > 0) detail += `：${body.trim().slice(0, 200)}`
    } catch {
  ...
  if (trimmed.length === 0) throw new Error('视觉辅助模型返回了空描述')
  return trimmed
}

/** Minimal attachment reference fields used for annotations. */
...
export function renderImageFailure(index: number, total: number, ref: ImageRefLike, reason: unknown): string {
